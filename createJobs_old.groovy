      ...
      file.withReader { reader ->
         while ((srcProp = reader.readLine())!=null) {
            ...
            Properties confProp = new Properties()
              File confPropertiesFile = new File("$WORKSPACE/iib/iib_Conf_old.properties")
...
//------Starting Deploy job --------                
                 freeStyleJob("${srcDetails.folderName}/${idx+1}-${srcDetails.serviceName}_${val}_Deploy") {
                   ...
                   steps {
                     if (("${val}" == "IG1" || "${val}" == "IG2" || "${val}" == "IG3" || "${val}" == "QA1" || "${val}" == "QA2" || "${val}" == "QA3")){
                  ...
                  parameters {
                        ...
                        if (("${val}" == "STRS" )){
                             choiceParam('MD_ENV', ['QA1', 'QA2', 'QA3'], 'Please select from Env: QA1/QA2/QA3')
                                      }
               				}
                  wrappers {
                         preBuildCleanup()
                  ...
                  println "${srcDetails.folderName}/${idx+1}-${srcDetails.serviceName}_${val}_Deploy Job Successfully Created."
                     ...
                     logRotator {
                      ...
                      parameters {
		                      ...
		                      if (("${val}" == "STRS" )){
		                            stringParam('ENV', depEnv."${val}", 'Please provide from Env: QA1/QA2/QA3')
		                           }else {
